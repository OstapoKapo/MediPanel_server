...
const csrfWhiteList = ['/auth/logIn', '/auth/verifyPassword']
...
export class CsrfMiddleware implements NestMiddleware {
    ...
    public with(redisService: RedisService): this {
  ...
  try {
    if (csrfWhiteListMethods.includes(req.method) && !csrfWhiteList.includes(req.path)) {
      ...
      if (!sessionId || !csrfHeader) {
        throw new ForbiddenException('Missing CSRF token or session id');
      }

      const session = await this.redisService.get<{ csrfToken: string }>(`session:${sessionId}`);
      console.log(sessionId, csrfHeader)
